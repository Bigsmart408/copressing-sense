...
# some upper limit
max_seed = 10 ** 9 + 7
#anomaly_score_example 函数根据原始数据和重建数据的差异来计算异常分数。
# 对于维度小于等于2的情况，将 d 除以所有维度的异常分数的倒数之和作为异常分数。对于维度大于2的情况，选择最大的两个异常分数的倒数，然后计算倒数的和的倒数作为异常分数。
...
#这个 window_sample_reconstruct 方法通过采样可信度对数据进行重建。
# 它使用采样率和采样可信度对原始数据进行采样，并根据分组信息将采样值进行重建。如果重建失败，会根据设定的重试限制逐步增加采样率，并进行重试。最后返回重建后的数据和重试次数。
...
def detect(data_in, data_out, start):
    ...
    # DEBUG
    # Chop down data size
    data = data.iloc[start:data.shape[0], 1:data.shape[1]]
